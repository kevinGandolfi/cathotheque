DELETE FROM books;


INSERT INTO
    books (title, first_edition)
VALUES
    ('Elisabeth', 1954);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Objectifs 1927-1928', 1927);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Fleurs récentes du divin parterre', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Le Saint Sacrifice de la Messe', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Entretiens de Saint François de Sales', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Erreurs et mensonges historiques', 1863);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Le vieillard', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Restauration française', 1850);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Réflexion sur la révolution de 1830', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    (
        'Simont de Montfort et la croisade contre les Albigeois',
        1910
    );


INSERT INTO
    books (title, first_edition)
VALUES
    ('Au service de l''amour', 1939);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Compagnons d''éternité', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    ('L''homme, cet inconnu', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    ('La prière de toutes les heures', 1941);


INSERT INTO
    books (title, first_edition)
VALUES
    ('L''Ame de tout apostolat', 1907);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Passion méditée au pied du St Sacrement', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    (
        'La franc-maçonnerie et la question religieuse',
        1892
    );


INSERT INTO
    books (title, first_edition)
VALUES
    (
        'Un prêtre redouté de Napoléon, le P. Bruno Lantéri',
        1955
    );


INSERT INTO
    books (title, first_edition)
VALUES
    ('Incarnation de l''homme', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Philosophie des moeurs contemporaines', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    ('L''intelligence en péril de mort', 1968);


INSERT INTO
    books (title, first_edition)
VALUES
    ('L''homme contre lui-même', 1962);


INSERT INTO
    books (title, first_edition)
VALUES
    (
        'La bienheureuse Jeanne d''Arc. Nouvelle vie populaire illustrée.',
        1907
    );


INSERT INTO
    books (title, first_edition)
VALUES
    ('Souvenirs', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    (
        'Le chrétien du jour et le chrétien de l''Evangile',
        NULL
    );


INSERT INTO
    books (title, first_edition)
VALUES
    ('Théorie du genre. Stop !', 2014);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Tout pour Jésus', 1854);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Introduction à la vie dévote', 1609);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Sainte Jeanne d''Arc à Vaucouleurs', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Les 3 conversions et les 3 voies', 1933);


INSERT INTO
    books (title, first_edition)
VALUES
    ('La révolution Française', 1928);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Comment j''élève mon enfant', 1927);


INSERT INTO
    books (title, first_edition)
VALUES
    ('La dévolution de la couronne de France', 1984);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Jésus-Christ roi de l''histoire', 1857);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Institutions liturgiques', 1840);


INSERT INTO
    books (title, first_edition)
VALUES
    ('La Voie Royale', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    (
        'La Présence Réelle du Verbe incarné dans les Espèces Consacrées',
        '1987'
    );


INSERT INTO
    books (title, first_edition)
VALUES
    ('Réflexion sur le Novus Ordo Missae', '2019');


INSERT INTO
    books (title, first_edition)
VALUES
    ('Le procès de la démocratie', 1971);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Histoires chinoises du Père Lebbe', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    (
        'La Dame des nations dans l''Europe catholique',
        1909
    );


INSERT INTO
    books (title, first_edition)
VALUES
    ('Paris révolutionnaire', 1920);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Vieilles maisons, vieux papiers', 1905);


INSERT INTO
    books (title, first_edition)
VALUES
    (
        'Histoire de la religion avant la venue du Christ',
        2023
    );


INSERT INTO
    books (title, first_edition)
VALUES
    ('La doctrine des lettres spirituelles', 1964);


INSERT INTO
    books (title, first_edition)
VALUES
    (
        'Les papes et la papauté de 1143 à la fin du monde d''après la prophétie attribuée à saint Malachie',
        1898
    );


INSERT INTO
    books (title, first_edition)
VALUES
    ('Le Christ Vie de l''âme', 1917);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Le Christ, idéal du moine', 1922);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Le Pacte de Reims', 1962);


INSERT INTO
    books (title, first_edition)
VALUES
    (
        'Les pierres sacrées / Poèmes de la tranchée',
        1922
    );


INSERT INTO
    books (title, first_edition)
VALUES
    (
        'La somme théologique de St. Thomas  en forme de catéchisme pour tous les fidèles',
        NULL
    );


INSERT INTO
    books (title, first_edition)
VALUES
    (
        'Lettres apostoliques de Pie IX, Grégoire XVI, Pie VII',
        NULL
    );


INSERT INTO
    books (title, first_edition)
VALUES
    (
        'Instructions pastorales sur les malheurs actuels de la France (Carême 1871)',
        NULL
    );


INSERT INTO
    books (title, first_edition)
VALUES
    ('Histoire de Jeanne d''Arc', 1890);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Pratique de la perfection chrétienne', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Explications de la thèse de Cassiciacum', 2011);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Evangile d''une grand''mère', 1866);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Histoire d''une âme', 1898);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Vers un ordre social chrétien', 1929);


INSERT INTO
    books (title, first_edition)
VALUES
    ('Saint François de Sales', NULL);


INSERT INTO
    books (title, first_edition)
VALUES
    (
        'Le Curé d''Ars, saint Jean-Marie Vianney (1786 - 1859)',
        1925
    );


INSERT INTO
    books (title, first_edition)
VALUES
    ('La Bienheureuse Marguerite de Lorraine', 1943);
